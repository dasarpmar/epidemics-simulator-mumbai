...
node_update_status update_infection(agent& node, int cur_time){
  ...
  if (node.infection_status==Progression::susceptible){
	...
	if(transition){
	  ...
	  infected_with_new_strain = bernoulli(node.lambda_higher/node.lambda);
	  if (infected_with_new_strain){
		  node.new_strain = true;
		  node.infectiousness *= GLOBAL.INFECTIOUSNESS_NEW_STRAIN;
	  }
	}
  }
	...
	//#pragma omp critical
	{if(node.new_strain){
		double probability_new_strain=pow(GLOBAL.VIRULENT_NEW_STRAIN,0.33)*STATE_TRAN[age_index][0];
	  transition = bernoulli(probability_new_strain);
	}else {
	...
	{
	  if(node.new_strain){
		double probability_new_strain=pow(GLOBAL.VIRULENT_NEW_STRAIN,0.33)*STATE_TRAN[age_index][1];
	  transition = bernoulli(probability_new_strain);
	}else {
	...
	{
	  if(node.new_strain){
		double probability_new_strain=pow(GLOBAL.VIRULENT_NEW_STRAIN,0.33)*STATE_TRAN[age_index][2];
	  transition = bernoulli(probability_new_strain);
	}else {
...
void updated_lambda_project(const vector<agent>& nodes, workplace& workplace){
  for(count_type i=0; i < workplace.projects.size(); ++i){
	  ...
	  double sum_value_project_higher = 0;
	  ...
	  workplace.projects[i].age_independent_mixing_higher = workplace.projects[i].scale*sum_value_project_higher;
  }  
}

void updated_lambda_w_age_independent(const vector<agent>& nodes, workplace& workplace){
  double sum_value = 0;
  double sum_value_higher = 0;
  ...
  for (count_type i=0; i < workplace.individuals.size(); ++i){
	...
	if (nodes[workplace.individuals[i]].new_strain){
		sum_value_higher += nodes[workplace.individuals[i]].lambda_w;
	}
  }
...
void updated_lambda_h_age_independent(const vector<agent>& nodes,  house& home){
  ...
  home.age_independent_mixing_higher =  home.scale*sum_value_higher;
}
  ...
  for(count_type i = 0; i < SIZE; ++i){
	...
	if(nodes[i].travels()){
	  ...
	  if(nodes[i].infective){
		  if(nodes[i].new_strain){
		infected_distance += nodes[i].commute_distance * mask_factor * GLOBAL.INFECTIOUSNESS_NEW_STRAIN;
		  }else{
...
double updated_travel_fraction_higher(const vector<agent>& nodes, const int cur_time){
  double infected_distance_higher = 0, total_distance = 0;
  count_type actual_travellers = 0, usual_travellers = 0;
...
#pragma omp parallel for default(none) shared(nodes) \
  ...
  for(count_type i = 0; i < SIZE; ++i){
	...
	if(nodes[i].travels()){
	  ...
	  ++actual_travellers;
	  total_distance += nodes[i].commute_distance;
	  if((nodes[i].infective)&&(nodes[i].new_strain)){
		infected_distance_higher += nodes[i].commute_distance * mask_factor  * GLOBAL.INFECTIOUSNESS_NEW_STRAIN;
	  }
	}
  }
  if(total_distance == 0 || usual_travellers == 0){
	  ...
	  return (infected_distance_higher/total_distance)
	* double(actual_travellers)/double(usual_travellers);
  }
...
void update_lambdas(agent&node, const vector<house>& homes, const vector<workplace>& workplaces, const vector<community>& communities, const vector<vector<nbr_cell>>& nbr_cells, const double travel_fraction, const double travel_fraction_higher, const int cur_time){
  ...
  node.lambda_incoming_higher.set_zero();
  ...
  node.lambda_higher = node.lambda_incoming_higher.sum();
...
#pragma omp parallel for default(none) shared(nodes, community) reduction (+: sum_value)
  for(count_type i = 0; i < SIZE; ++i){
	...
	if (nodes[community.individuals[i]].new_strain){			 
	sum_value_higher
	  += nodes[community.individuals[i]].lambda_c
	  * std::min(community.w_c,
				 nodes[community.individuals[i]].neighborhood_access_factor);
...
#pragma omp parallel for default(none) shared(houses, nodes)
  for(count_type i = 0;  i < HOUSES_SIZE; ++i){
	...
	houses[i].lambda_random_community_outgoing_higher = lambda_random_community_outgoing_higher;
  }
#pragma omp parallel for default(none) shared(houses, communities)
  for(count_type i = 0; i < HOUSES_SIZE; ++i){
	double sum_value_household = 0;
	double sum_value_household_higher = 0;
...
#pragma omp parallel for default(none)					\
  ...
  reduction (+: sum_values)
		...
		for(count_type k=0; k<houses[house_index].individuals.size(); ++k){
		  sum_values += nodes[houses[house_index].individuals[k]].lambda_nbr_cell
			* std::min(communities[houses[house_index].community].w_c,
			...
			if (nodes[houses[house_index].individuals[k]].new_strain){
				sum_values_higher += nodes[houses[house_index].individuals[k]].lambda_nbr_cell
			        * std::min(communities[houses[house_index].community].w_c,
					   houses[house_index].neighborhood_access_factor);
			}				
		}
	  ...
	  nbr_cells[i][j].lambda_nbr_higher = nbr_cells[i][j].scale*sum_values_higher;
	}
  }
}
...
void update_lambda_c_global(vector<community>& communities,
  ...
  for (count_type c1 = 0; c1 < SIZE; ++c1){
	...
	for (count_type c2 = 0; c2 < SIZE; ++c2){
	  ...
	  num_higher += fk_val * communities[c2].lambda_community_higher;
	  denom += fk_val;
	}
	if(denom==0){		
...
#pragma omp parallel for default(none) shared(nodes, community)			\
  reduction(+: errors,													\
			...
			hd_area_recovered_from_vaccination)
  for (count_type i=0; i<SIZE; ++i){
