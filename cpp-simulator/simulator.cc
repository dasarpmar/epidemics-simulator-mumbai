...
plot_data_struct run_simulation(){
  ...
  std::string logging1 = "Time_step,total_new_infections,new_strain_infections,active_patients,active_patients_new_strain" ;
  std::vector<std::string> logger1;
  logger1.push_back(logging1);

  std::vector<count_type> new_vaccinated_candidates;
  new_vaccinated_candidates.reserve(GLOBAL.num_people);
  std::vector<count_type> new_vaccinated_candidates_45;
  new_vaccinated_candidates_45.reserve(GLOBAL.num_people);
  ...
  for(count_type time_step = 0; time_step < GLOBAL.NUM_TIMESTEPS; ++time_step){
	...
	count_type num_new_strain_infections = 0;
	...
	for(count_type j = 0; j < NUM_PEOPLE; ++j){
		 ...
		 if ((time_step == 1632)||(time_step == 1660)||(time_step == 1688)||(time_step == 1716)||(time_step == 1752)||(time_step == 1780)||(time_step == 1808)||(time_step == 1836)){
           if ((nodes[j].age>=60)&&(!nodes[j].vaccinated)){
	  ...
	  if(node_update_status.new_infection){
		...
		if (nodes[j].new_strain){
		++num_new_strain_infections;
		}
...
#pragma omp parallel for default(none)									\
  shared(travel_fraction, time_step, homes, workplaces, communities, nbr_cells, nodes)
	...
	count_type num_active_infections_new_strain = 0;
  ...
  reduction(+: n_infected, n_exposed,									\
	...
	for(count_type j = 0; j < NUM_PEOPLE; ++j){
	  ...
	  if(infection_status == Progression::exposed){
		...
		if (nodes[j].new_strain){
		num_active_infections_new_strain += 1;
		}
	  }
	...
	logging1 = std::to_string(time_step)+","+std::to_string(num_new_infections)+","+std::to_string(num_new_strain_infections)+","+std::to_string(num_active_infections)+","+std::to_string(num_active_infections_new_strain)+",";
    logger1.push_back(logging1);
